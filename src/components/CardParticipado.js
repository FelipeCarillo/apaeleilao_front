...
export default function CardParticipados({data}){
    ...
    function abrirMapa () {
        window.open("https://maps.app.goo.gl/K97x1sZdQziXugSh7", "mozillaTab");
    }
    ...
    //Verificação se está pago para o mapa
    function verificaPago (status_payment) {
        if (status_payment === 'PENDING'){
            ...
            allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade" className="mx-auto w-[100%] h-[200px] md:w-[70%] md:h-[300px]"></iframe>
        }
    }
            ...
            <div>
                ...
                <div key={index} className="border-2 border-black rounded-[25px]">
                    ...
                    <div className="text-center">
                        ...
                        { data.status_payment === 'PENDING' ? <button className="bg-yellow-300 p-2 border-2 border-black rounded-[45px] w-[80%] my-3 text-xl hover:text-2xl" onClick={() => {openModal(data)}}>Pagar</button>
                        : data.status_payment === 'PAID' ? <button className="bg-verde p-2 border-2 border-black rounded-[45px] w-[80%] my-3 text-white text-xl hover:text-2xl" onClick={() => {openModal(data)}}> <i className="fa-solid fa-check"></i> </button> 
                        : <button className="bg-vermelho p-2 border-2 border-black rounded-[45px] w-[80%] my-3 text-white text-xl hover:text-2xl" onClick={() => {openModal(data)}}> <i className="fa-solid fa-x"></i> </button>}
                    </div>
                </div>
                {/* Modal */}
                <div>
                  ...
                  >
                    ...
                    <div className="flex justify-between bg-azul p-2 mb-5 md:mb-10">
                        ...
                        <button className="text-4xl w-[40px] text-vermelho hover:bg-vermelho hover:text-white rounded-tr-lg" onClick={closeModal}><i className="fa-solid fa-xmark"></i></button>
                    </div>
                    {/* Conteudo */}

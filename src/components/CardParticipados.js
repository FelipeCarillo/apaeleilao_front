export default function CardParticipados({...props}){
    ...
    return (
        <div className="border-2 border-black rounded-2xl shadow-2xl">
            <img className="rounded-t-2xl w-full" src="http://via.placeholder.com/500x500" alt="IMAGEM"/>
            <div className="p-2 text-center flex flex-col items-center">
                ...
                <p className={`${props.podio === '' ? "hidden" : 'block'}`}>Pódio: {props.podio}</p>
                ...
                <label className={`cursor-pointer px-4 py-2 w-3/4 rounded-full ${props.stage === 'B' ? 'bg-blue-500 text-white font-medium text-xl': 'hidden'}`}>Participou</label>
            </div>
        </div>
    )
}